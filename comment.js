                                    ...
                                    if(data.status == "success") {
                                          ...
                                          var comment = $('<div class="comments-block author-info"  id="post_comment_'+data.result.id+'"> \n' +
                                                ...
                                                '</div> \n' +
                                                 ...
                                                 ' <form method="POST" action="/main/send_comment"  id="comment-form_'+data.result.news_id+'" class="comment-form validate-form" role="form"> \n' +
                                                 ' <input type="hidden" name="news_id" value="'+data.result.news_id+'">\n' +
                                                  ...
                                                  ' <textarea  id="texarea-comment_'+data.result.news_id+'" class="form-control input-field textarea textarea-comment"   cols="30" rows="10" name="message" placeholder="Комментарий *"></textarea>\n' +
                                                  ' <button class="fa fa-paper-plane-o comment-submit pull-right" onclick="clickSubmit('+data.result.news_id+');return false;" type="submit"></button>\n' +
                                                  ' </div>\n' +
